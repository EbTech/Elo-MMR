...
fn main() {
    ...
    println!("CodeForces average performance ({} contests, top-50):", max_contests);
    for si in -20..20 {
        for wi in 1..40 {
            ...
            for (i, contest_id) in contest_ids.iter().enumerate() {
                ...
                avg_perf += predict_performance(&mut players, &contest, &system);
                simulate_contest(&mut players, &contest, &mut system);
            }
    ...
    for pi in -25..25 {
        for li in -25..25 {
            let sig_perf = (pi as f64) * 5. + 250.;
            let sig_limit = (li as f64) * 2.5 + 100.;

            players.clear();
            let now = Instant::now();
            let mut system = EloRSys {sig_perf: sig_perf, sig_limit: sig_limit, logistic: true};
            let mut avg_perf = 0.;
    ...
    for ei in 1..10 {
        for bi in -5..5 {
            for si in 0..10 {
                ...
                let sigma_growth = (si as f64) * 5. + 10.;
