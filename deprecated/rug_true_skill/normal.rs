...
impl Gaussian {
    ...
    pub fn leq_eps(&self, eps: &Float) -> Gaussian {
        ...
        let (mu, sigma) = if alpha == 0 {
            (eps.clone(), eps.clone())
        } else {
            let mu = (moment1(&self.mu, &self.sigma, &neg_eps) - moment1(&self.mu, &self.sigma, &eps)) / &alpha;
            let sigma2 = (moment2(&self.mu, &self.sigma, &neg_eps)
                - moment2(&self.mu, &self.sigma, &eps)) / &alpha
                - mu.clone().square();
            // sigma2 can only be negative due to numerical errors
            (mu, sigma2.max(&to_hp(0.)).sqrt());
        }
