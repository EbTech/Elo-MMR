...
struct Player {
    ...
    int conservativeRating() const { // displayed rating
        return round(posterior.mu - 2*(posterior.sig - sig_limit));
    }
...
void simulateCodeforcesHistory() {
    ...
    for (int roundNum : contests) {
        ...
        cerr << "Processing contest " << roundNum;
    ...
    for (pair<const string,Player>& entry : players) {
        ...
        conservativeRatings.push_back(make_tuple(player.conservativeRating(), entry.first,
            player.maxRating, player.prevRating, round(player.perfs.back().mu), player.prevContest));
        sumRatings += r.mu;
    }
...
int main() {
    //testRobustness(); // produces data to plot in the paper
    simulateCodeforcesHistory(); // takes a long time to run
}
